...
When you run `VACUUM (FULL)`, `CLUSTER`, or `pg_repack`, PostgreSQL resets the counters in `pg_stat_user_tables` for that table. This is actually helpful—it gives you a clean baseline.

The validation process:

1. **Note the timestamp** before your rewrite operation
2. **Run the rewrite** (`VACUUM (FULL)`, `CLUSTER`, or `pg_repack`)
...
**The Fix:**
- Run the rewrite operation (`VACUUM (FULL)`, `CLUSTER`, or `pg_repack`) *after* setting fillfactor.
- Choose the least disruptive approach for your workload:
  - **`VACUUM (FULL)`**: Simplest, but locks the table (use during maintenance windows).
  - **`CLUSTER`**: Locks but also optimizes physical order on disk (good if you have a natural sort key).
  - **`pg_repack`**: Non-blocking external tool (requires installation, but safest for uptime).
...
- Ran `pg_repack` on Sunday evening (non-blocking)
